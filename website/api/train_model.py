...
# 2. Limpiamos coordenadas y hora — descartamos filas con datos faltantes
...
# 3. Convertimos las columnas Con_* a binario (SI → 1, cualquier otra cosa → 0)
con_cols = [c for c in df_ml.columns if str(c).startswith("Con_")]
for col in con_cols:
    df_ml[col] = (df_ml[col] == "SI").astype(int)

# 4. Convertimos el nombre del mes a número
mapa_meses = {
